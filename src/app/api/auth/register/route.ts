...
export async function POST(req: NextRequest) {
  try {
    ...
    const db = getDb();
    const existing = await db`SELECT id FROM users WHERE username = ${lower}`;
    if (existing.length > 0) return NextResponse.json({ error: 'Username already taken' }, { status: 409 });

    const passwordHash = await bcrypt.hash(password, 12);
    const [user] = await db`INSERT INTO users (username, password_hash) VALUES (${lower}, ${passwordHash}) RETURNING id, username`;
    await db`INSERT INTO profiles (user_id, display_name) VALUES (${user.id}, ${lower})`;
    await db`INSERT INTO view_counts (user_id, total_views) VALUES (${user.id}, 0)`;
